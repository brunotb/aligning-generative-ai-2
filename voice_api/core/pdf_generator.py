...
The module:
1. Transforms voice form answers (keyed by voice field_id) to PDF format (keyed by pdf_field_id)
2. Handles type conversions (choice field strings to integers)
3. Fills the PDF template with the transformed data
4. Returns the PDF as bytes for transmission or storage
"""
...
def transform_answers_to_pdf_format(voice_answers: Dict[str, str]) -> Dict[str, Any]:
    ...
    for field_id, value in voice_answers.items():
        ...
        if not field_def:
            # Skip unknown fields silently (graceful fallback)
            continue
        ...
        if field_def.validator.type == "integer_choice":
            try:
                ...
                # If conversion fails, store as string (graceful fallback)
...
def generate_anmeldung_pdf(
    ...
    Raises:
        ...
        ValueError: If transformation of answers fails
        Exception: If PDF filling or saving fails
    ...
    Example:
        ...
        ...     "gender_p1": "0",
        ... }
        >>> pdf_bytes = generate_anmeldung_pdf(answers, "output/form.pdf")
        >>> len(pdf_bytes) > 0
        True
    """
